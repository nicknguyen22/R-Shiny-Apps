...
glmnet_initial <- c("naomit","dow", "dummy") # <-- These are arbitrary starting values. Set these to your best recommendation
pls_initial <- c("impute_knn","dow","scale","center","dummy")   # <-- These are arbitrary starting values. Set these to your best recommendation
rf_initial <- c("impute_knn","dow","pls", "interact","scale","center","dummy")
qrf_initial <- c("impute_knn","dow","pls", "interact","scale","center","dummy") 
lm_initial <- c("impute_knn","dow","pls","scale","center","dummy") 
rlm_initial <- c("impute_knn","dow","pls","scale","center","dummy")
glm_initial <- c("naomit","dow","pls","scale","center","dummy")
lasso_initial <- c("naomit","dow","pls","scale","center","dummy")
knn_initial <- c("impute_knn","dow","pls","scale","center","dummy")
bag_initial <- c("impute_knn","dow","pls","scale","center","dummy")
nnet_initial <- c("impute_knn","dow","scale","center","dummy") 
svml_initial <- c("impute_knn","dow","scale","center","dummy")
svmr_initial <- c("impute_bag","dow","scale","center","dummy") 
svmrc_initial <- c("impute_bag","dow","scale","center","dummy")
svmrs_initial <- c("impute_bag","dow","scale","center","dummy")
svmp_initial <- c("impute_bag","dow","scale","center","dummy")
krlsr_initial <- c("impute_bag","dow","scale","center","dummy")
krlsp_initial <- c("impute_knn","dow","scale","center","dummy") 
rpart_initial <- c("impute_knn", "pls", "interact") # <-- These are arbitrary starting values. Set these to your best recommendation
# maintenance point ---------------------------------------------------------------------------------------------------------------------------
...
ppchoices <- c("impute_knn", "impute_bag", "impute_median", "impute_mode", "YeoJohnson", "naomit", "pca", "pls", "ica", "center", "scale", "month", "year", "dow", "dateDecimal", "nzv", "zv","other", "dummy","poly", "interact", "indicate_na", "BoxCox","unknown")
...
dynamicSteps <- function(recipe, preprocess) {
  ...
  for (s in preprocess) {
    if (s == "impute_knn") {
      ...
      recipe <- recipes::step_naomit(recipe, all_predictors(), skip = T)
    } else if (s == "pca") {
      ...
      recipe <- recipes::step_date(recipe, has_type("date"), features = c("year"), ordinal = FALSE) # add step_date year
    } else if (s == "month") {
      ...
      recipe <- recipes::step_unknown(recipe, all_nominal_predictors())
    } else if (s == "dummy") {
